  ...
  <aside class="fixed! h-screen">
    <div class="sidebar" :class="{ 'is-expanded': isExpanded }">
      <div class="sidebar-content">
        ...
        <!-- menu -->
        <el-menu
          class="menu flex flex-1 flex-col overflow-hidden"
...
.sidebar {
  ...
  > .sidebar-content {
    @apply flex-start flex h-full w-full flex-col;
  }
  ...
  .divider {
    @apply mx-auto my-4 w-[24px] border-[var(--border-primary)];
  }
...
.sidebar.is-expanded {
  ...
  .logo-box {
    @apply w-full justify-start pb-0 pl-7;
...
.add-playlist-button {
  @apply flex h-full flex-1 cursor-pointer items-center text-[var(--navigation-text)];
}

.expanded-button {
  @apply cursor-pointer rounded-full bg-[var(--alpha-bg)] p-2;
}
</style>
